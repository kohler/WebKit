...
unsigned RenderTable::effectiveSpanOfColumn(const RenderTableCol& column) const
{
    if (!m_columnRenderersValid)
        updateColumnCache();
    if (!column.firstChild())
        return column.span();
    // |column| is a column group with children, so its effective span
    // is the sum of its childrens' spans.
...
LayoutUnit RenderTable::offsetWidthForColumn(const RenderTableCol& column) const
{
    ...
    unsigned span = effectiveSpanOfColumn(column);
