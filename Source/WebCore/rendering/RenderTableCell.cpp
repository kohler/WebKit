...
static LayoutRect backgroundRectForSection(const RenderTableSection& section, const RenderTable& table)
{
    LayoutRect rect = { { }, section.size() };
    if (!table.collapseBorders()) {
        auto hSpacing = table.hBorderSpacing();
        auto vSpacing = table.vBorderSpacing();
        // All sections' size() contains an unwanted vSpacing at the
        // block-end position. The first section's size() contains an
        // additional unwanted vSpacing at the block-start position. All
        // sections' size() contains unwanted hSpacing on both inline ends.
        auto beforeBlockSpacing = &section == table.topSection() ? vSpacing : 0_lu;
