    ...
    BleedAvoidance bleedAvoidance, const InlineIterator::InlineBoxIterator& inlineBoxIterator, CompositeOperator op, RenderElement* backgroundObject, BaseBackgroundColorUsage baseBgColorUsage) const
{
    ...
    if (!baseBgColorOnly && shouldPaintBackgroundImage) {
        // m_imageRect is the primary rect used for sizing the image, which is different from the rect for painting. It is set for table rows and for inline boxes (multiline inline boxes paint like the image was one long strip spanning lines).
        auto geometry = [&] () -> BackgroundImageGeometry {
            if (m_imageRect)
                return calculateBackgroundImageGeometry(m_renderer, m_paintInfo.paintContainer, bgLayer, m_imageRect->location(), scrolledPaintRect, m_imageRect);
            return calculateBackgroundImageGeometry(m_renderer, m_paintInfo.paintContainer, bgLayer, rect.location(), scrolledPaintRect);
        }();
...
BackgroundImageGeometry BackgroundPainter::calculateBackgroundImageGeometry(const RenderBoxModelObject& renderer, const RenderLayerModelObject* paintContainer, const FillLayer& fillLayer, const LayoutPoint& paintOffset, const LayoutRect& borderBoxRect, std::optional<LayoutRect> imageRect)
{
    ...
    if (fixedAttachment) {
        LayoutPoint attachmentPoint = destinationRect.location();
    ...
    if (imageRect) {
        ...
        // expand repeating background to cover whole borderBoxRect in repeat
        // directions
        if (backgroundRepeatX == FillRepeat::Repeat)
